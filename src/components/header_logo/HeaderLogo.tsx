...
const HeaderLogo = memo(
    function HeaderIcon({ prSizeText = 100, prIsTextHorizontal = true } : PropsHeaderIcon)
    {
        const [ stLogoColours, setLogoColours ] = useState<string[]>(
            utils.getFromLocalStorageTyped<string[]>(glclStrgKeyLogoColours, (v): v is string[] => Array.isArray(v)) || 
            [ Block.getRandomColour(), Block.getRandomColour() ]
        );
        ...
        const lOnClick = useCallback(
            () =>
            {
                ...
                utils.setInLocalStorage(glclStrgKeyLogoColours, lColoursNew);
